    ...
    try:
        ...
        for rel in graph.computation_relationships.values():
            if rel.relation_type == ComputationRelationType.DEPENDS_ON:
                ...
                # Store datasource property name for input mapping
                output_prop = None
                if hasattr(rel, "datasource") and rel.datasource is not None:
                    output_prop = rel.datasource.property_name
                ...
                # Store data_output property name for output mapping
                output_prop = None
                if hasattr(rel, "data_output") and rel.data_output is not None:
                    output_prop = rel.data_output.property_name
        ...
        for node_id in order:
            ...
            if node_data.get("is_computation"):
                ...
                try:
                    ...
                    # Update successors with output using edge property_name
        ...
        for node_id in order:
            ...
            if node_data.get("is_computation"):
                ...
                try:
                    ...
                    # Update successors using edge property_name
                    for successor in G.successors(node_id):
                        edge_data = G.edges[node_id, successor]
                        if edge_data.get("relation_type") == "OUTPUT_TO":
                            property_name = edge_data.get("property_name")
